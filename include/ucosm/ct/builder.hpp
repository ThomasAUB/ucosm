...
namespace ucosm {
    ...
    namespace detail {
        ...
        template<typename L>
        struct tlist_to_topology;
        template<typename... Es>
        struct tlist_to_topology<tlist<Es...>> { using type = ugraph::Topology<Es...>; };

        template<typename A, typename B>
        struct tlist_concat;
        template<typename... A, typename... B>
        struct tlist_concat<tlist<A...>, tlist<B...>> { using type = tlist<A..., B...>; };

        // Filter out void types from a tlist
        template<typename T>
        struct tlist_filter_voids;
        template<>
        struct tlist_filter_voids<tlist<>> { using type = tlist<>; };
        template<typename... Ts>
        struct tlist_filter_voids<tlist<void, Ts...>> { using type = typename tlist_filter_voids<tlist<Ts...>>::type; };
        template<typename H, typename... Ts>
        struct tlist_filter_voids<tlist<H, Ts...>> {
            using rest = typename tlist_filter_voids<tlist<Ts...>>::type;
            using type = typename tlist_concat<tlist<H>, rest>::type;
        };
    }
*/
