...
- **Backend**: the platform hooks are a `constexpr TaskletBackend` with static storage, passed as a template parameter - `TaskletScheduler<interrupt_count, backend> sched;`. The hooks are resolved at compile time: a null hook costs nothing and a provided one is called directly, and a missing `getTick` is a compile error.
