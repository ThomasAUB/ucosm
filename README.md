...
- **Backend**: the platform hooks are a `TaskletBackend`, passed to the constructor and copied - `TaskletScheduler<interrupt_count> sched(backend);`. `getTick` is required. The other hooks are optional : a null one is replaced by a no-op at construction, so that each hook costs an indirect call without a test.
